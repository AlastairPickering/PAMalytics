...
# ---------------------- helpers (module-level, unchanged) ----------------------
...
# ---------------------- required callable ----------------------
def render_settings(df, sources) -> None:
    """
    Entry-point used by the PAM shims. `df` and `sources` are provided by the Dashboard.
    We don’t actually need them for this page, but we accept them for a consistent API.
    """
    # Safe page config (won’t error if already set elsewhere)
    try:
    ...
    # Resolve defaults from session AFTER title so flash is minimal
    ...
    if src_mode == "Use filename-level CSV/XLSX":
        ...
        with col_f2:
            ...
            st.write("Found" if p.exists() else "Not found")
        ...
        with c0:
            tau = st.number_input(
                "Threshold (tau)", min_value=0.0, max_value=1.0, value=tau_default, step=0.01,
...
# Allow running this file directly as a Streamlit page
if __name__ == "__main__":
    render_settings(df=None, sources=None)
