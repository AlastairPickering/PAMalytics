...
def load_and_preprocess(file_path, target_length):
    # Load audio using librosa; returns mono waveform and sample rate
    waveform, sr = librosa.load(str(audio_file), sr=None)
    if waveform.ndim > 1:  # stereo to mono
...
def extract_embedding(file_path, target_length, beats_model, device):
...
def extract_embedding_from_array(waveform: np.ndarray,
                                 sr: int,
                                 target_samples: int,
                                 beats_model,
                                 device):
    """
    Pad/trim an in-memory waveform to target_samples (samples), then
    run through BEATs and mean-pool to get numpy embedding.
    """
    # pad/trim
    wav = pad_or_trim_waveform(waveform, target_samples)
    # to tensor and extract
